...
class wpl_flex_controller extends wpl_controller
{
	...
	private function save_dbst()
	{
		...
		/** echo response **/
		echo json_encode(array('success'=>1, 'message'=>__('Field saved.', 'wpl'), 'data'=>NULL));
		exit;
	}
	...
	private function remove_dbst($dbst_id)
	{
		...
		{
			$success = 0;
			$message = __('Field is not deletable.', 'wpl');
		}
    ...
    private function convert_dbst()
	{
		...
		/** echo response **/
		echo json_encode(array('success'=>1, 'message'=>__('Field converted.', 'wpl'), 'data'=>NULL));
		exit;
	}
