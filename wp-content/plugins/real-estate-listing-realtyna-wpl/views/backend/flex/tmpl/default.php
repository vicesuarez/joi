...
<div class="wrap wpl-wp flex-wp<?php echo ($this->kind == 2 ? ' user-flex': ''); ?>">
    <header>
        ...
        <h2><?php echo sprintf(__('%s Data Structure', 'wpl'), ucfirst($this->kind_label)); ?></h2>
    </header>
    ...
    <div class="sidebar-wp">
        ...
        <div class="wpl-side-2 side-tabs-wp">
            <ul>
				...
				<li><a href="#<?php echo $category->id; ?>" class="wpl_slide_label wpl_slide_label_prefix_<?php echo $category->prefix; ?>" id="wpl_slide_label_id<?php echo $category->id; ?>" onclick="rta.internal.slides.open('<?php echo $category->id; ?>', '.side-tabs-wp', '.wpl_slide_container', 'currentTab');"><?php echo __($category->name, 'wpl'); ?></a></li>
                <?php endforeach; ?>
            </ul>
        </div>
