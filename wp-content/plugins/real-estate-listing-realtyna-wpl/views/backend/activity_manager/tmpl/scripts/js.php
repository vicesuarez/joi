	...
	wplj(".sortable_activity").sortable(
    {
        ...
        update: function(e, ui)
        {
            ...
            wplj.ajax(
            {
                ...
                {
                    wpl_show_messages(data + ' <?php echo addslashes(__('Activity Sorted!', 'wpl')); ?>', '.wpl_activity_manager_list .wpl_show_message', 'wpl_green_msg');
                },
                error: function(jqXHR, textStatus, errorThrown)
                {
                    wpl_show_messages('<?php echo addslashes(__('Error Occured.', 'wpl')); ?>', '.wpl_activity_manager_list .wpl_show_message', 'wpl_red_msg');
                }
            });
        }
    });
...
function wpl_set_enabled_activity(activity_id, enabled_status)
{
	if (!activity_id)
	{
		wpl_show_messages("<?php echo addslashes(__('Invalid Activity', 'wpl')); ?>", '.wpl_activity_manager_list .wpl_show_message');
		return false;
	}
...
function wpl_remove_activity(activity_id, confirmed)
{
	...
	if (!confirmed)
	{
		message = "<?php echo addslashes(__('Are you sure you want to remove this item?', 'wpl')); ?>&nbsp;(<?php echo addslashes(__('ID', 'wpl')); ?>:" + activity_id + ")&nbsp;<?php echo addslashes(__('All related items will be removed.', 'wpl')); ?>";
		message += '<span class="wpl_actions" onclick="wpl_remove_activity(' + activity_id + ', 1);"><?php echo addslashes(__('Yes', 'wpl')); ?></span>&nbsp;<span class="wpl_actions" onclick="wpl_remove_message();"><?php echo addslashes(__('No', 'wpl')); ?></span>';
...
function wpl_generate_modify_activity_page(activity_id)
{
	...
	else
	{
		...
		if(!activity_name)
		{
			wpl_show_messages('<?php echo addslashes(__('Please select activity to import!', 'wpl')); ?>', '.wpl_activity_manager_list .wpl_show_message', 'wpl_red_msg');
			return;
		}
