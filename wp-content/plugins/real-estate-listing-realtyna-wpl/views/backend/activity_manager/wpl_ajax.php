...
class wpl_activity_manager_controller extends wpl_controller
{
    ...
    public function display()
    {
        ...
        // Check Nonce
        if(!wpl_security::verify_nonce(wpl_request::getVar('_wpnonce', ''), 'wpl_activity_manager')) $this->response(array('success'=>0, 'message'=>__('The security nonce is not valid!', 'wpl')));
    ...
    private function remove_activity($activity_id, $confirmed = 0)
    {
        ...
        $message = $res ? __('Activity removed successfully.', 'wpl') : __('Error Occured.', 'wpl');
        $data = NULL;
    ...
    private function load_options($activity_name, $current_layout)
    {
        ...
        else $returnData['options'] = __("This activity doesn't have options!", 'wpl');
