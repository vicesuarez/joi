...
function add_to_wpl(user_id)
{
	if(!user_id)
	{
		wpl_show_messages("<?php echo addslashes(__('Invalid user', 'wpl')); ?>", '.wpl_user_list .wpl_show_message');
		return false;
	}
...
function wpl_remove_user(user_id, confirmed)
{
	...
	if(!confirmed)
	{
		message = "<?php echo addslashes(__('Are you sure you want to remove this item?', 'wpl')); ?>&nbsp;(<?php echo addslashes(__('ID', 'wpl')); ?>:"+user_id+")&nbsp;<?php echo addslashes(__('All related items will be removed.', 'wpl')); ?>";
		message += '&nbsp;<span class="wpl_actions" onclick="wpl_remove_user(\''+user_id+'\', 1);"><?php echo addslashes(__('Yes', 'wpl')); ?></span>&nbsp;<span class="wpl_actions" onclick="wpl_remove_message();"><?php echo addslashes(__('No', 'wpl')); ?></span>';
...
function wpl_save_user()
{
	...
	wplj.ajax(
	{
		...
		success: function(data)
		{
			wpl_show_messages('<?php echo addslashes(__('Membership add.', 'wpl')); ?>', '.wpl_data_structure_list .wpl_show_message', 'wpl_green_msg');
			wplj._realtyna.lightbox.close();
		},
		error: function(jqXHR, textStatus, errorThrown)
		{
