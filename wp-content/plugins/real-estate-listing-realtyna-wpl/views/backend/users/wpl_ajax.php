...
class wpl_users_controller extends wpl_controller
{
	...
	public function display()
	{
        ...
        // Check Nonce
        if(!wpl_security::verify_nonce(wpl_request::getVar('_wpnonce', ''), 'wpl_users')) $this->response(array('success'=>0, 'message'=>__('The security nonce is not valid!', 'wpl')));
	...
	private function add_user_to_wpl($user_id)
	{
		...
		$res = (int) $res;
		$message = $res ? __('User added to WPL successfully.', 'wpl') : __('Error Occured.', 'wpl');
		$data = NULL;
	...
	private function del_user_from_wpl($user_id, $confirmed = 0)
	{
		...
		$res = (int) $res;
		$message = $res ? __('User removed from WPL successfully.', 'wpl') : __('Error Occured.', 'wpl');
		$data = NULL;
