...
class wpl_settings_controller extends wpl_controller
{
	...
	public function display()
	{
        ...
        // Check Nonce
        if(!wpl_security::verify_nonce(wpl_request::getVar('_wpnonce', ''), 'wpl_settings')) $this->response(array('success'=>0, 'message'=>__('The security nonce is not valid!', 'wpl')));
	...
	private function save_watermark_image()
	{
		...
		{
			$error = __('An error ocurred uploading your file.', 'wpl');
		}
		else 
		{
			// check the extention
	...
	private function clear_cache()
	{
		...
		$res = 1;
		$message = $res ? __('Operation was successful.', 'wpl') : __('Error Occured.', 'wpl');
		$data = NULL;
    ...
    private function remove_upload()
    {
        ...
        $response = array('success'=>1, 'message'=>__('Uploaded file removed successfully!', 'wpl'));
    ...
    private function generate_language_keywords()
    {
		...
		$res = 1;
		$message = $res ? __('Language strings are generated.', 'wpl') : __('Error Occured.', 'wpl');
		$data = NULL;
    ...
    private function clear_calendar_data()
    {
        ...
        $message = $res ? __('Calendar Data removed.', 'wpl') : __('Error Occured.', 'wpl');
        $data = NULL;
    ...
    private function import_settings()
    {
		...
		{
			$error = '';
        	$message = __('Settings have been imported successfuly!', 'wpl');
		}
        else
        {
        	$error = '1';
        	$message = __('Cannot import settings!', 'wpl');
        }
