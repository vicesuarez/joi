...
$addons[14] = array('name'=>'Mobile App', 'id'=>'6', 'addon_name'=>'mobile_application', 'description'=>'A professional Real Estate app, custom branded and totally integrated to your WPL-based website.', 'readmore_link'=>'http://wpl.realtyna.com/redirect.php?action=download&item=mobile_application', 'button_text'=>'More Info', 'addon_tag'=>'Recommended');
$addons[15] = array('name'=>'Booking Add On', 'id'=>'20', 'addon_name'=>'booking', 'description'=>'Enables the customers to book the vacation rental properties directly through the website.', 'readmore_link'=>'http://wpl.realtyna.com/redirect.php?action=download&item=booking', 'button_text'=>'More Info', 'addon_tag'=>'');
$addons[16] = array('name'=>'Calendar Add On', 'id'=>'13', 'addon_name'=>'calendar', 'description'=>'Availability info on calendar for vacation rental listings.', 'readmore_link'=>'http://wpl.realtyna.com/redirect.php?action=download&item=calendar', 'button_text'=>'More Info', 'addon_tag'=>'');
$addons[17] = array('name'=>'Neighborhoods Add On', 'id'=>'18', 'addon_name'=>'neighborhoods', 'description'=>'Define hierarchically neighborhoods using this addon and assign certain listings to related neighborhood.', 'readmore_link'=>'http://wpl.realtyna.com/redirect.php?action=download&item=neighborhoods', 'button_text'=>'More Info', 'addon_tag'=>'');
$addons[18] = array('name'=>'Review Add On', 'id'=>'22', 'addon_name'=>'review', 'description'=>'This add-on enables customers to rate the features of properties which can be arranged through backend by admin.', 'readmore_link'=>'http://wpl.realtyna.com/redirect.php?action=download&item=review', 'button_text'=>'More Info', 'addon_tag'=>'');
                ...
                <?php $i = 0; foreach($addons as $addon): if(wpl_global::check_addon($addon['addon_name'])) continue; $i++; ?>
                    <div class="wpl-ni-addon-row wpl_ni_addon_container" id="wpl_ni_addons_container<?php echo $addon['id']; ?>">
                        <div class="wpl_ni_addon_subject">
                            ...
                            <?php if(trim($addon['addon_tag']) != '') echo '<span class="wpl_ni_addon_tag">'.__($addon['addon_tag'], 'wpl').'</span>'; ?>
                        </div>
                        <div class="wpl_ni_addon_description"><?php echo $addon['description']; ?></div>
                        <a class="readmore_link" href="<?php echo $addon['readmore_link']; ?>" target="_blank"><?php echo __($addon['button_text'], 'wpl'); ?></a>
                    </div>
                <?php endforeach; ?>
                <?php if($i == 0): ?>
