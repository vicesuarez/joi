...
<div class="video-tabs-wp" id="gallery-tabs-wp-container">
	<ul>
        <li id="wpl_gallery_uploader_tab" onclick="wpl_gallery_select_tab('wpl_gallery_uploader_tab', 'wpl_gallery_uploader'); return false;" class="active"><a href="#wpl_gallery_uploader"><?php echo __('Image uploader', 'wpl'); ?></a></li>
		<li id="wpl_gallery_external_tab" onclick="wpl_gallery_select_tab('wpl_gallery_external_tab', 'wpl_gallery_external'); return false;"><a href="#wpl_gallery_external"><?php echo __('External images', 'wpl'); ?></a></li>
	</ul>
</div>
...
<div class="gallary-btn-wp">
    <div id="wpl_gallery_uploader" class="wpl_gallery_method_container">
        <div class="wpl-button button-1 button-upload">
            <span><?php echo __('Select files', 'wpl'); ?></span>
    ...
    <div id="wpl_gallery_external" class="wpl_gallery_method_container" style="display: none;">
        ...
        <div class="field-desc">
            <?php echo __('Pro addon must be installed for this!', 'wpl'); ?>
        </div>
        <?php else: ?>
        ...
        <div id="wpl_gallery_external_cnt" style="margin-top: 10px; display: none;">
            <div class="gallery-external-wp" id="gallery-external-cnt">
                <div class="row">
                    <label for="gallery_external_link"><?php echo __('Image links', 'wpl'); ?></label>
                    <textarea name="gallery_external_link${count}" rows="8" cols="50" id="gallery_external_link" placeholder="<?php echo __('Enter each image link in a new line', 'wpl'); ?>"></textarea>
                    <button class="wpl-button button-1" onclick="wpl_gallery_external_save();"><?php echo __('Save', 'wpl'); ?></button>
                </div>
            </div>
        </div>
        <?php endif; ?>
...
<div id="files" class="gallary-images-wp wpl_files_container">
    <div class="wpl-mass-actions">
        <button class="wpl-button button-1" onclick="wpl_gallery_delete_all();"><?php echo __('Delete all images', 'wpl'); ?></button>
    </div>
        ...
        foreach($gall_items as $image)
		{
            ...
            <li class="ui-state-default" id="ajax_gallery<?php echo $image->index; ?>" >
                ...
                <div class="image-box-wp">
                    ...
                    <div class="info-wp">
                        <div class="row">
                            <label for=""><?php echo __('Image Title', 'wpl') ?>:</label>
                        ...
                        <div class="row">
                            <label for=""><?php echo __('Image Description', 'wpl'); ?>:</label>
                        ...
                        <div class="row">
                            <label for=""><?php echo __('Image Category', 'wpl'); ?>:</label>
    ...
    <?php
		...
		foreach ($image_categories as $img_cat) {
			$image_categories_html .= ' <option value="' . $img_cat->category_name . '">' . __($img_cat->category_name, 'wpl') . '</option>';
		}
    ?>
</div>
...
wplj(document).ready(function()
{
    ...
    wplj('#fileupload').fileupload(
    {
            ...
            wplj(data.result.files).each(function(index, file)
            {
                if (file.error !== undefined)
                {
                    ...
                    var hbHTML     = hbTemplate({
                        index: img_counter,
                        name: file.name,
                        enabled_title: "<?php echo addslashes(__('Enabled', 'wpl')) ?>",
        ...
        {
            wplj('#progress_img').hide();
            wplj("#error_ajax_img").html("<span color='red'><?php echo addslashes(__('Error occured', 'wpl')) ?> : " + data.files[data.index].name + " " + data.files[data.index].error + "</span>");
...
{
	if(!confirm("<?php echo addslashes(__('Are you sure?', 'wpl')); ?>")) return;
...
function wpl_image_enabled(gallery, id)
{
	...
	{
		if (status == 0)
			wplj("#active_image_tag_" + id).html('<i class="action-btn icon-disabled" title="<?php echo addslashes(__('Disabled', 'wpl')); ?>"></i>');
		else
			wplj("#active_image_tag_" + id).html('<i class="action-btn icon-enabled" title="<?php echo addslashes(__('Enabled', 'wpl')); ?>"></i>');
	});
}
...
<div class="attach-btn-wp">
	...
	<div class="field-desc">
		<?php echo __('To attach files click on the "Select files" button.', 'wpl'); ?>
	</div>
</div>
...
<div id="attaches" class="attachment-wp wpl_files_container">
    <div class="wpl-mass-actions">
        <button class="wpl-button button-1" onclick="wpl_attachment_delete_all();"><?php echo __('Delete all attachments', 'wpl'); ?></button>
    </div>
    ...
    foreach ($att_items as $attachment)
    {
        ...
        <li class="ui-state-default" id="ajax_attachment<?php echo $attachment->index; ?>">
            ...
            <div class="image-box-wp">
                ...
                <div class="info-wp">
                    <div class="row">
                        <label for=""><?php echo __('Attachment Title', 'wpl') ?>:</label>
                    ...
                    <div class="row">
                        <label for=""><?php echo __('Attachment Description', 'wpl') ?>:</label>
                    ...
                    <div class="row">
                        <label for=""><?php echo __('Attachment Category', 'wpl') ?>:</label>
...
wplj(document).ready(function()
{
    ...
    wplj('#attachment_upload').fileupload(
    {
        ...
        processfail: function (e, data)
        {
            ...
            wplj("#error_ajax_att").html("<span color='red'><?php echo __('Error occured', 'wpl') ?> : " + data.files[data.index].name + " " + data.files[data.index].error + "</span>");
...
{
	ajax = wpl_run_ajax_query("<?php echo wpl_global::get_full_url(); ?>", "wpl_format=b:listing:attachments&wpl_function=title_update&pid=<?php echo $item_id; ?>&attachment="+attachment+"&value="+value+"&kind=<?php echo $this->kind; ?>&_wpnonce=<?php echo $nonce; ?>");
}

function ajax_attachment_desc_update(attachment, value)
{
	ajax = wpl_run_ajax_query("<?php echo wpl_global::get_full_url(); ?>", "wpl_format=b:listing:attachments&wpl_function=desc_update&pid=<?php echo $item_id; ?>&attachment="+attachment+"&value="+value+"&kind=<?php echo $this->kind; ?>&_wpnonce=<?php echo $nonce; ?>");
}

function ajax_attachment_cat_update(attachment, value)
{
	ajax = wpl_run_ajax_query("<?php echo wpl_global::get_full_url(); ?>", "wpl_format=b:listing:attachments&wpl_function=cat_update&pid=<?php echo $item_id ?>&attachment="+attachment+"&value="+value+"&kind=<?php echo $this->kind; ?>&_wpnonce=<?php echo $nonce; ?>");
}

function ajax_attachment_delete(attachment, id)
{
	if(confirm('<?php _e('Are you sure?', 'wpl') ?>'))
	{
...
function wpl_attachment_enabled(attachment, id)
{
	...
	ajax.success(function (data)
	{
		if (status == 0) wplj("#active_attachment_tag_" + id).html('<i class="action-btn icon-disabled wpl_actions_btn wpl_show" title="<?php echo addslashes(__('Disabled', 'wpl')); ?>"></i>');
		else wplj("#active_attachment_tag_" + id).html('<i class="action-btn icon-enabled wpl_actions_btn wpl_show" title="<?php echo addslashes(__('Enabled', 'wpl')); ?>"></i>');
	});
}
</script>
<?php
...
<div class="video-tabs-wp" id="video-tabs">
	<ul>
		<li class="active"><a id="embed-tab" href="#embed" onclick="video_select_tab(0); return false;"><?php echo __('Embed code', 'wpl'); ?></a></li>
		<?php if(wpl_settings::get('video_uploader')): ?>
		<li><a id="uploader-tab" href="#uploader" onclick="video_select_tab(1); return false;"><?php echo __('Video uploader', 'wpl'); ?></a></li>
		<?php endif; ?>
	</ul>
</div>
...
<div class="content-wp" id="embed">
	<button class="wpl-button button-1" onclick="add_embed_video();"><?php echo __('Add video', 'wpl') ?></button>
	<?php foreach ($vid_embed_items as $vid_embed_item): ?>
    <div class="video-embed-wp" id="video-embed-<?php echo $vid_embed_count; ?>">
        <div class="row">
            <label id="title_label" for="embed_vid_title<?php echo $vid_embed_count; ?>"><?php echo __('Video title', 'wpl'); ?></label>
        ...
        <div class="row">
            <label id="desc_label" for="embed_vid_desc<?php echo $vid_embed_count; ?>"><?php echo __('Video description', 'wpl'); ?></label>
        ...
        <div class="row">
            <label id="embed_label" for="embed_vid_code<?php echo $vid_embed_count; ?>"><?php echo __('Video embed code', 'wpl'); ?></label>
...
function add_embed_video()
{
	var embedVideo = rta.template.bind({
		...
		title: "<?php echo addslashes(__('Video title', 'wpl')); ?>",
		desc: "<?php echo addslashes(__('Video description', 'wpl')); ?>",
		embedCode: "<?php echo addslashes(__('Video embed code', 'wpl')); ?>",
		item_name: '',
		item_extra1: '',
		item_extra2: '',
		id: '-1'
...
{
	if (confirm("<?php echo addslashes(__('Are you sure?', 'wpl')); ?>"))
    {
...
<div class="content-wp wpl-util-hidden" id="uploader">
	...
	<div id="video" class="video-list-wp wpl_files_container">
			...
			foreach ($vid_items as $video)
			{
				...
				<li class="ui-state-default" id="ajax_video<?php echo $video->index; ?>">
					...
					<div class="image-box-wp">
						<div class="info-wp">
							<div class="row">
								<label for=""><?php echo __('Video Title', 'wpl'); ?>:</label>
							...
							<div class="row">
								<label for=""><?php echo __('Video Description', 'wpl'); ?>:</label>
							...
							<div class="row">
								<label for=""><?php echo __('Video Category', 'wpl'); ?>:</label>
		...
		foreach ($video_categories as $vid_cat)
		{
			$video_categories_html .= ' <option value="' . $vid_cat->category_name . '">' . __($vid_cat->category_name, 'wpl') . '</option>';
		}
		?>
	</div>
</div>
...
wplj(document).ready(function()
{
    ...
    wplj('#video_upload').fileupload(
    {
            ...
            wplj(data.result.files).each(function (index, file)
            {
                ...
                else
                {
					...
					var hbHTML     = hbTemplate({
						...
						lblTitle: "<?php echo addslashes(__('Video Title', 'wpl')); ?>",
        ...
        processfail: function (e, data)
        {
            ...
            wplj("#error_ajax_vid").html("<span color='red'><?php echo __('Error occured', 'wpl'); ?> : " + data.files[data.index].name + " " + data.files[data.index].error + "</span>");
