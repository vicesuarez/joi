...
class wpl_notifications
{
    ...
    public function set_recipients($recipients)
    {
        ...
        elseif($this->mode == 'sms')
        {
            ...
            foreach($recipients as $recipient)
            {
                ...
                if(is_numeric($recipient) and $recipient >= 0)
                {
                    $user_data = $this->sms->wpl_get_user_data('`u`.`ID`,`wpl`.`mobile`', "AND wpl.`mobile`='".$recipient."'", 'loadObject');
    ...
    {
        wpl_file::write(self::get_template_path($data['template_path'], true), stripslashes($data['template']));
