...
elseif($type == 'text' and !$done_this) //////////////////////////// text ////////////////////////////
{
    if(trim($value) != '')
	{
        ...
        if(isset($options['if_zero']) and $options['if_zero'] == 2 and $value == '0') $return['value'] = __($options['call_text'], 'wpl');
...
elseif($type == 'select' and !$done_this) //////////////////////////// select ////////////////////////////
{
	if(trim($value) and trim($value) != '-1')
	{
		...
		{
			if($value == $field_option['key']) $return['value'] = __($field_option['value'], 'wpl');
		}
	}
	...
	if($values[$field->table_column] != 0) 
	{
		...
		if(isset($options['values']))
		{
			foreach($options['values'] as $field_option)
			{
				...
				{
					$return['values'][$i] = __($field_option['value'], 'wpl');
					$i++;
				}
			}
		}
...
elseif($type == 'property_types' and !$done_this) //////////////////////////// property types ////////////////////////////
{
	if(trim($value) != '0' or trim($value) != '-1')
	{
		...
		$return['type'] = $field->type;
		$return['name'] = __($field->name, 'wpl');
		$return['value'] = is_object($property_type) ? __($property_type->name, 'wpl') : NULL;
	}
...
elseif($type == 'listings' and !$done_this) //////////////////////////// listings ////////////////////////////
{
	if(trim($value) != '0' or trim($value) != '-1')
	{
		...
		$return['type'] = $field->type;
		$return['name'] = __($field->name, 'wpl');
		$return['value'] = is_object($listing_type) ? __($listing_type->name, 'wpl') : NULL;
	}
...
elseif($type == 'mmnumber' and !$done_this) //////////////////////////// Min/Max numbers ////////////////////////////
{
    if(trim($value) != '')
	{
        ...
        if(isset($options['if_zero']) and $options['if_zero'] == 2 and !trim($value) and !trim($values[$field->table_column.'_max'])) $return['value'] = __($options['call_text'], 'wpl');
...
elseif($type == 'locations' and !$done_this) //////////////////////////// Locations ////////////////////////////
{
	...
	for($i=1; $i<=7; $i++)
	{
		...
		$return['locations'][$i] = __($values['location'.$i.'_name'], 'wpl');
	...
	if(isset($values['zip_name']) and trim($values['zip_name']))
	{
		...
		$return['locations']['zips'] = __($values['zip_name'], 'wpl');
        $return['raw']['zips'] = $values['zip_name'];
		$return['keywords']['zips'] = __($location_settings['locationzips_keyword'], 'wpl');
	}
...
elseif($type == 'price' and !$done_this) //////////////////////////// Price ////////////////////////////
{
    ...
    if($field->table_column == 'price' and wpl_global::check_addon('calendar'))
    {
        ...
        {
            $return['value'] = __('From', 'wpl').' '.$return['value'];
        }
    }
...
elseif($type == 'mmprice' and !$done_this) //////////////////////////// Min/Max Price ////////////////////////////
{
    ...
    if(isset($options['if_zero']) and $options['if_zero'] == 2 and !trim($value) and !trim($value_max)) $return['value'] = __($options['call_text'], 'wpl');
