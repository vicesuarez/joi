...
class wpl_pagination
{
    ...
    public function show()
	{
        if(!$this->calculate and $this->calculate())
		{
            ...
            if(isset($this->show_total) and $this->show_total) echo '<span class="wpl_total_result">' . $this->total_pages . ' ' . __('Results returned.', 'wpl') . '</span>';
            if(isset($this->show_page_size) and $this->show_page_size)
			{
                ...
                echo '<span class="wpl_page_size_title">' . __('Per Page ', 'wpl') . '</span>';
    ...
    public function getOutput()
	{
        if(!$this->calculate and $this->calculate())
		{
            ...
            if($this->show_total) $string .= '<span class="wpl_total_result">' . $this->total_pages . ' ' . __('Results returned.', 'wpl') . '</span>';
            if($this->show_page_size)
			{
                $page_sizes = explode(',', trim(wpl_global::get_setting('page_sizes'), ', '));

                $string .= '<span class="wpl_page_size">';
                $string .= '<span class="wpl_page_size_title">' . __('Per Page ', 'wpl') . '</span>';
    ...
    public static function get_pagination($num_result, $page_size = '', $show_options = false, $js_link = 0)
	{
        ...
        $p->nextLabel(__("Next", 'wpl'));
        $p->prevLabel(__("Previous", 'wpl'));
