...
if($format == 'select' and !$done_this)
{
	if($value != '-1' and trim($value) != '')
    {
        ...
        if($table_column == 'user_id' and wpl_global::check_addon('multi_agents'))
        {
...
elseif($format == 'unit' and !$done_this)
{
	if($value != '-1' and trim($value) != '')
	{
        ...
        $min = (isset($vars['sf_min_'.$table_column]) and $vars['sf_min_'.$table_column] != '-1') ? $vars['sf_min_'.$table_column] : 0;
		$max = (isset($vars['sf_max_'.$table_column]) and $vars['sf_max_'.$table_column] != '-1') ? $vars['sf_max_'.$table_column] : 0;
