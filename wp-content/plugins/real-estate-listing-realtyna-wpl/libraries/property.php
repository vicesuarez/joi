...
class wpl_property
{
	...
	public static function generate_default_query($fields)
	{
        ...
        /** If only one country is enabled **/
        if(wpl_db::num("SELECT COUNT(id) FROM `#__wpl_location1` WHERE `enabled`='1'") == 1)
        {
            $country = wpl_db::select("SELECT `id`, `name` FROM `#__wpl_location1` WHERE `enabled`='1' LIMIT 1", 'loadAssoc');
	...
	public static function render_property($property, $fields, &$finds = array(), $material = false)
	{
		...
		foreach($fields as $key=>$field)
		{
			...
			if(isset($field->accesses) and trim($field->accesses) != '' and wpl_global::check_addon('membership'))
			{
                ...
                {
                    $rendered[$field->id] = array('field_id'=>$field->id, 'type'=>$field->type, 'name'=>__($field->name, 'wpl'), 'value'=>__($field->accesses_message, 'wpl'));
                    if($material and trim($field->table_column) != '') $materials[$field->table_column] = array('field_id'=>$field->id, 'type'=>$field->type, 'name'=>__($field->name, 'wpl'), 'value'=>__($field->accesses_message, 'wpl'));
			...
			if(!$done_this)
			{
				$rendered[$field->id] = array('field_id'=>$field->id, 'type'=>$field->type, 'name'=>__($field->name, 'wpl'), 'value'=>$value);
                if($material and trim($field->table_column) != '') $materials[$field->table_column] = array('field_id'=>$field->id, 'type'=>$field->type, 'name'=>__($field->name, 'wpl'), 'value'=>$value);
			}
		}
    ...
    public static function render_field($value, $dbst_id)
    {
        ...
        if(trim($field->accesses) != '')
        {
            ...
            elseif(!in_array($cur_membership_id, $accesses) and trim($field->accesses_message) != '')
            {
                $return = array('field_id'=>$field->id, 'type'=>$field->type, 'name'=>__($field->name, 'wpl'), 'value'=>__($field->accesses_message, 'wpl'));
            }
        }
    ...
    public static function render_markers($wpl_properties)
    {
        ...
        foreach($wpl_properties as $key=>$property)
        {
            ...
            {
                $markers[$i]['title'] = wpl_render::render_price($property['raw']['price'], $property['raw']['price_unit'], '', wpl_global::wpl_minimize_price($property['raw']['price']));
            }
	...
	public static function finalize($property_id, $mode = 'edit', $user_id = '')
	{
        ...
        if(wpl_global::check_multilingual_status())
        {
            ...
            if(!$alias_multilingual = wpl_addon_pro::get_multiligual_status_by_column('alias', $kind)) wpl_property::update_alias(NULL, $property_id);
            if(!$page_title_multilingual = wpl_addon_pro::get_multiligual_status_by_column('field_312', $kind)) wpl_property::update_property_page_title($property);
            if(!$title_multilingual = wpl_addon_pro::get_multiligual_status_by_column('field_313', $kind)) wpl_property::update_property_title($property);
            ...
            foreach($languages as $language)
            {
                if(wpl_global::switch_language($language))
                {
                    ...
                    if($alias_multilingual) wpl_property::update_alias(NULL, $property_id);
                    if($page_title_multilingual) wpl_property::update_property_page_title($property);
                    if($title_multilingual) wpl_property::update_property_title($property);
        ...
        {
            wpl_property::update_text_search_field($property_id);
            wpl_property::update_alias(NULL, $property_id);
	...
	public static function update_text_search_field($property_id)
	{
		...
		foreach($rendered as $data)
		{
			...
			{
				$value = $data['name'] .(isset($data['distance']) ? ' ('. $data['distance'] .' '. __('MINUTES', 'wpl') .' '. __('BY', 'wpl') .' '. $data['by'] .')' : '');
			}
			elseif($type == 'feature')
			{
	...
	public static function generate_location_text($property_data, $property_id = 0, $glue = ',', $force = false)
	{
		...
		if(isset($property_data[$street_no_column]) and trim($property_data[$street_no_column]) != '') $locations['street_no'] = __($property_data[$street_no_column], 'wpl');
        ...
        if(isset($property_data[$street_column]) and trim($property_data[$street_column]) != '') $locations['street'] = __($property_data[$street_column], 'wpl');
        
        if(isset($property_data['location7_name']) and trim($property_data['location7_name']) != '') $locations['location7_name'] = __($property_data['location7_name'], 'wpl');
        if(isset($property_data['location6_name']) and trim($property_data['location6_name']) != '') $locations['location6_name'] = __($property_data['location6_name'], 'wpl');
        if(isset($property_data['location5_name']) and trim($property_data['location5_name']) != '') $locations['location5_name'] = __($property_data['location5_name'], 'wpl');
        if(isset($property_data['location4_name']) and trim($property_data['location4_name']) != '') $locations['location4_name'] = __($property_data['location4_name'], 'wpl');
        if(isset($property_data['location3_name']) and trim($property_data['location3_name']) != '') $locations['location3_name'] = __($property_data['location3_name'], 'wpl');
        if(isset($property_data['location2_name']) and trim($property_data['location2_name']) != '') $locations['location2_name'] = __($property_data['location2_name'], 'wpl');
        if(isset($property_data['zip_name']) and trim($property_data['zip_name']) != '') $locations['zip_name'] = __($property_data['zip_name'], 'wpl');
        if(isset($property_data['location1_name']) and trim($property_data['location1_name']) != '') $locations['location1_name'] = __($property_data['location1_name'], 'wpl');
        
        // Get the pattern
        $default_pattern = '[street_no] [street][glue] [location4_name][glue] [location3_name][glue] [location2_name][glue] [location1_name] [zip_name]';
	...
	public static function update_alias($property_data, $property_id = 0, $glue = '-', $force = false)
	{
		...
		if(trim($property_data['property_type'])) $alias['property_type'] = __(wpl_global::get_property_types($property_data['property_type'])->name, 'wpl');
		if(trim($property_data['listing'])) $alias['listing'] = __(wpl_global::get_listings($property_data['listing'])->name, 'wpl');
        
        if(trim($property_data['location1_name'])) $alias['location1'] = __($property_data['location1_name'], 'wpl');
        if(trim($property_data['location2_name'])) $alias['location2'] = __($property_data['location2_name'], 'wpl');
        if(trim($property_data['location3_name'])) $alias['location3'] = __($property_data['location3_name'], 'wpl');
        if(trim($property_data['location4_name'])) $alias['location4'] = __($property_data['location4_name'], 'wpl');
        if(trim($property_data['location5_name'])) $alias['location5'] = __($property_data['location5_name'], 'wpl');
        if(trim($property_data['zip_name'])) $alias['zipcode'] = __($property_data['zip_name'], 'wpl');
        
		$alias['location'] = self::generate_location_text($property_data, $property_id, '-');
		
		if(trim($property_data['rooms'])) $alias['rooms'] = $property_data['rooms'].__('Room'.($property_data['rooms'] > 1 ? 's': ''), 'wpl');
		if(trim($property_data['bedrooms'])) $alias['bedrooms'] = $property_data['bedrooms'].__('Bedroom'.($property_data['bedrooms'] > 1 ? 's': ''), 'wpl');
		if(trim($property_data['bathrooms'])) $alias['bathrooms'] = $property_data['bathrooms'].__('Bathroom'.($property_data['bathrooms'] > 1 ? 's': ''), 'wpl');
	...
	public static function update_property_page_title($property_data, $property_id = 0, $force = false)
	{
        ...
        // Get the pattern
        $default_pattern = '[property_type] [listing] [glue] [rooms] [glue] [bedrooms] [glue] [bathrooms] [glue] [price] [glue] [id] [glue]';
		...
		/** apply filters **/
		_wpl_import('libraries.filters');
		@extract(wpl_filters::apply('generate_property_page_title', array('title_str'=>$title_str, 'title'=>$title, 'property_data'=>$property_data)));
    ...
    public static function update_property_title($property_data, $property_id = 0, $force = false)
	{
		...
		/** apply filters **/
		_wpl_import('libraries.filters');
		@extract(wpl_filters::apply('generate_property_title', array('title_str'=>$title_str, 'title'=>$title, 'property_data'=>$property_data)));
	...
	public static function full_render($property_id, $plisting_fields = NULL, $property = NULL, $params = array(), $force = false)
	{
        ...
        elseif(!trim($raw_data[$column]) and wpl_settings::get('cache')) $rendered = json_decode(wpl_property::generate_rendered_data($property_id), true);
	...
	public static function import($properties_to_import, $wpl_unique_field = 'mls_id', $user_id = '', $source = 'mls', $finalize = true, $log_params = array())
	{
		...
		foreach($properties_to_import as $property_to_import)
		{
			...
			foreach($property_to_import as $key=>$row)
			{
				...
				$q .= "`$wpl_field`='".wpl_db::escape(utf8_decode(wp_check_invalid_utf8($wpl_value)))."',";
			}
    ...
    public static function get_meta_keywords($property_data, $property_id = 0, $force = false)
	{
        ...
        if($property_data['meta_keywords_manual'] or (isset($property_data[$column]) and trim($property_data[$column]) != '' and !$force)) return stripslashes($property_data[$column]);
        ...
        if(wpl_db::columns('wpl_properties', $column))
        {
            $query = "UPDATE `#__wpl_properties` SET `".$column."`='".$keywords_str."' WHERE `id`='".$property_id."'";
            wpl_db::q($query, 'update');
        }
        ...
        if($base_column and wpl_db::columns('wpl_properties', $base_column))
        {
            $query = "UPDATE `#__wpl_properties` SET `".$base_column."`='".$keywords_str."' WHERE `id`='".$property_id."'";
            wpl_db::q($query, 'update');
        }
    ...
    public static function get_meta_description($property_data, $property_id = 0, $force = false)
	{
        ...
        if($property_data['meta_description_manual'] or (isset($property_data[$column]) and trim($property_data[$column]) != '' and !$force)) return stripslashes($property_data[$column]);
        ...
        if(wpl_db::columns('wpl_properties', $column))
        {
            $query = "UPDATE `#__wpl_properties` SET `".$column."`='".$description."' WHERE `id`='".$property_id."'";
            wpl_db::q($query, 'update');
        }
        ...
        if($base_column and wpl_db::columns('wpl_properties', $base_column))
        {
            $query = "UPDATE `#__wpl_properties` SET `".$base_column."`='".$description."' WHERE `id`='".$property_id."'";
            wpl_db::q($query, 'update');
        }
    ...
    public static function clear_property_thumbnails($property_id)
    {
        ...
        $property = wpl_db::select("SELECT `id`, `kind` FROM `#__wpl_properties` WHERE `id`='{$property_id}'", 'loadAssoc');
        $ext_array = array('jpg', 'jpeg', 'gif', 'png');
    ...
    public static function clone_listing($property_id)
    {
        ...
        $forbidden_fields = array('id', 'kind', 'mls_id', 'pic_numb', 'att_numb', 'sent_numb', 'contact_numb', 'user_id', 'visit_time',
            'visit_date', 'last_modified_time_stamp', 'sp_featured', 'sp_hot','vids_numb', 'geopoints', 'blog_id');
