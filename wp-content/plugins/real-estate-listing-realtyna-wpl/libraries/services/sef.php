...
class wpl_service_sef
{
	...
	public function set_profile_page_params($user_id)
	{
		...
		$this->user_title = '';
		$this->user_keywords = '';
		$this->user_description = __('Listings of', 'wpl');
		...
		$this->user_title .= ' '.__('Listings', 'wpl');
		$this->user_keywords = trim($this->user_keywords, ', ');
		$this->user_description .= ' '.__('which is located in', 'wpl').' '.$user_data['location_text'];
    ...
    public function set_payments_page_params()
    {
		...
		/** set title **/
		$html->set_title(__('Payments', 'wpl'));
    }
