...
class wpl_service_helps
{
    ...
    public function show_help_tab($contextual_help, $screen_id, $screen)
    {
        ...
        if(!isset($tabs['sidebar'])) $tabs['sidebar'] = array('content'=>'<a class="wpl_contextual_help_tour button" href="'.wpl_global::add_qs_var('wpltour', 1).'">'.__('Introduce Tour', 'wpl').'</a>');
    ...
    public function generate_scripts($tips = array())
    {
        ...
        wplj(document).ready(function()
        {
            ...
            var wpltip<?php echo $tip['id']; ?> =
            {
                ...
                {
                    var button = wplj('<a class="wpl-pointer-close button-secondary wpl-btn-close"><?php echo __('Close', 'wpl'); ?></a>');
