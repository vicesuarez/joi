...
function wpl_flex_add_feature_param()
{
	var html = '<div class="fanc-row" id="wpl_flex_select_params_row' + wpl_flex_param_counter + '">' +
            ...
            '<label for="<?php echo $__prefix; ?>opt_values[' + wpl_flex_param_counter + '][value]"><?php echo __('Option', 'wpl'); ?> '+wpl_flex_param_counter+'</label>' +
...
<div class="fanc-body">
	...
	<div class="col-wp">
		...
		<div class="col-fanc-right" id="wpl_flex_specific_options">
			...
			?>
            <div class="fanc-row fanc-inline-title">
				<span><?php echo __('Params', 'wpl'); ?></span>
                <span class="action-btn icon-plus margin-left-1p" onclick="wpl_flex_add_feature_param();"></span>
			</div>
            <div class="fanc-row">
				<label for="<?php echo $__prefix; ?>opt_type"><?php echo __('Type', 'wpl'); ?></label>
                <select name="<?php echo $__prefix; ?>opt_type" id="<?php echo $__prefix; ?>opt_type">
                    <option value="none" <?php echo ((isset($options['type']) and $options['type'] == 'none') ? 'selected="selected"' : ''); ?>><?php echo __('None', 'wpl'); ?></option>
                    <option value="single" <?php echo ((isset($options['type']) and $options['type'] == 'single') ? 'selected="selected"' : ''); ?>><?php echo __('Single', 'wpl'); ?></option>
                    <option value="multiple" <?php echo ((isset($options['type']) and $options['type'] == 'multiple') ? 'selected="selected"' : ''); ?>><?php echo __('Multiple', 'wpl'); ?></option>
                </select>
			</div>
            <?php
			$i = 1;
            ...
            <div class="fanc-row" id="wpl_flex_select_params_row<?php echo $i; ?>">
                ...
                <label for="<?php echo $__prefix; ?>opt_values[<?php echo $i; ?>][value]"><?php echo __('Option', 'wpl').' '.$i; ?></label>
