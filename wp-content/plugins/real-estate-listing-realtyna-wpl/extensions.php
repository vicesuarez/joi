...
class wpl_extensions
{
	...
	{
        $locale = apply_filters('plugin_locale', get_locale(), 'wpl');
        
		$overriden_language_filepath = WP_LANG_DIR .DS. WPL_BASENAME .DS. 'wpl'.'-'.$locale.'.mo';
        ...
        {
            load_textdomain('wpl', WP_LANG_DIR .DS. WPL_BASENAME .DS. 'wpl'.'-'.$locale.'.mo');
        }
		else
        {
			load_plugin_textdomain('wpl', false, WPL_BASENAME . DS . 'languages');
        }
	}
	...
	public function import_sidebar($extension)
	{
		...
		register_sidebar(array(
			'name'          => __($name, 'wpl'),
			'id'            => $id,
			'description'   => __($description, 'wpl'),
	...
	public function wpl_admin_menus()
	{
		...
		if(wpl_users::is_administrator($cur_user_id) or isset($cur_user_data->data->wpl_data->id))
		{
			...
			foreach($submenus as $submenu)
			{
				...
				$menu_title = $submenu->separator ? $controller->wpl_add_separator().__($submenu->menu_title, 'wpl') : __($submenu->menu_title, 'wpl');
				
				add_submenu_page($submenu->parent, __($submenu->page_title, 'wpl'), $menu_title, $role, $submenu->menu_slug, array($controller, $submenu->function));
			}
		}
	}
	...
	public function wp_title($title)
	{
		...
		if(in_array($wplview, array('property_listing', 'profile_listing')) and $wplpage >= 2)
		{
            if(is_string($title) and trim($title) != '')
            {
                ...
                return wpl_global::clean($title.' -- '.__('Page', 'wpl').' '.$wplpage);
            }
            elseif(is_array($title))
            {
                $title['title'] .= ' -- '.sprintf(__('Page %s', 'wpl'), $wplpage);
            }
		}
    ...
    public static function wpl_plugin_links($links, $file)
    {
        ...
        {
            $links[] = '<a href="'.wpl_global::get_wp_admin_url().'admin.php?page=wpl_admin_settings">'.__('Settings', 'wpl').'</a>';
            $links[] = '<a href="http://wpl.realtyna.com/wassets/wpl-manual.pdf" target="_blank">'.__('WPL Manual', 'wpl').'</a>';
            $links[] = '<a href="http://wpl.realtyna.com/redirect.php?action=shop" target="_blank">'.__('WPL Add-ons', 'wpl').'</a>';
        }
