...
class wpl_global
{
	...
	public static function min_access($role = 'guest', $user_id = '')
	{
		...
		{
			echo __("You don't have access to this page!", 'wpl');
			exit;
		}
	}
	...
	public static function upload($file, $dest = '', $ext_array = array('jpg','png','gif','jpeg'), $max_file_size = 512000, $extension = NULL)
	{
		...
		else
		{
			...
			{
				$error .= __('File extension is not valid.', 'wpl');
			}
			...
			{
				$error .= __('File size is not valid!', 'wpl');
				@unlink($file);
			}
	...
	public static function check_addon_update($addon_id)
	{
		...
		if(!$addon_data) return array('success'=>0, 'message'=>__('Error: #U200, Addon is not valid!', 'wpl'));
		if(!$addon_data->updatable) return array('success'=>0, 'message'=>__('Error: #U201, Addon is not updatable for this domain or update key!', 'wpl'));
		...
		$message = $answer['update'] ? __('A new update found. please wait ...', 'wpl') : __('Your addon is up to date!', 'wpl');
    ...
    public static function get_current_language()
    {
        ...
        return apply_filters('plugin_locale', get_locale(), 'wpl');
    }
    ...
    public static function get_upload_abs($type = 'path')
    {
        ...
        if($type == 'path')
        {
            ...
            if(wpl_global::is_multisite())
            {
                $ex = explode('/sites/', $dir);
                $ex = explode('\sites\\', $ex[0]);
    ...
    public static function switch_language($language)
    {
        ...
        $result = wpl_global::load_textdomain('wpl', $path);
    ...
    public static function add_to_realtyna_mailing_list($email)
	{
		$POST = array(
			'email'=>$email,
            'password'=>md5($email),
			'command'=>'add_to_mailing_list',
			'format'=>'json'
		);
        ...
        if($status == '0') $message = __('An error occurred!', 'wpl');
        elseif($status == '1') $message = __('Your email added to our mailing list.', 'wpl');
        elseif($status == '2') $message = __('Your email already added.', 'wpl');
	...
	public static function wpl_minimize_price($price)
	{
		...
		elseif($price >= 1000 and $price < 1000000) return round($price/1000).'K';
    ...
    public static function render_pattern($the_pattern, $property_id, $property_data, $glue = ',', $data = NULL)
    {
        ...
        {
            $data = array();
            $data['property_type'] = __($property_type, 'wpl');
            $data['listing'] = __($listing, 'wpl');

            if(trim($property_data['location1_name'])) $data['location1'] = __($property_data['location1_name'], 'wpl');
            if(trim($property_data['location2_name'])) $data['location2'] = __($property_data['location2_name'], 'wpl');
            if(trim($property_data['location3_name'])) $data['location3'] = __($property_data['location3_name'], 'wpl');
            if(trim($property_data['location4_name'])) $data['location4'] = __($property_data['location4_name'], 'wpl');
            if(trim($property_data['location5_name'])) $data['location5'] = __($property_data['location5_name'], 'wpl');
            if(trim($property_data['location6_name'])) $data['location6'] = __($property_data['location6_name'], 'wpl');
            if(trim($property_data['location7_name'])) $data['location7'] = __($property_data['location7_name'], 'wpl');
            if(trim($property_data['zip_name'])) $data['zipcode'] = __($property_data['zip_name'], 'wpl');

            if(trim($property_data['rooms'])) $data['rooms'] = $property_data['rooms'].' '.__('Room'.($property_data['rooms'] > 1 ? 's': ''), 'wpl');
            if(trim($property_data['bedrooms'])) $data['bedrooms'] = $property_data['bedrooms'].' '.__('Bedroom'.($property_data['bedrooms'] > 1 ? 's': ''), 'wpl');
            if(trim($property_data['bathrooms'])) $data['bathrooms'] = $property_data['bathrooms'].' '.__('Bathroom'.($property_data['bathrooms'] > 1 ? 's': ''), 'wpl');

            $data['price'] = __('Price', 'wpl').' '.wpl_render::render_price($property_data['price'], $property_data['price_unit']);
        ...
        foreach($matches_pattern[1] as $pattern)
        {
            ...
            if(strpos($pattern, 'str:') !== false)
            {
                ...
                if($condition and ((isset($data[$condition]) and trim($data[$condition])) or (isset($property_data[$condition]) and trim($property_data[$condition])))) $rendered = str_replace('[' . $pattern . ']', __($text, 'wpl'), $rendered);
                ...
                else $rendered = str_replace('[' . $pattern . ']', __($text, 'wpl'), $rendered);
            }
            // Abbreviation
            elseif(strpos($pattern, 'abbr:') !== false)
            {
                $location_level = trim($pattern, 'abbr:location');
                $rendered = str_replace('[' . $pattern . ']', wpl_locations::get_location_abbr_by_name($data['location'.$location_level], $location_level), $rendered);
            }
            // Pattern exists on data
            elseif(isset($data[$pattern]))
            {
                $rendered = str_replace('[' . $pattern . ']', $data[$pattern], $rendered);
            }
            elseif(isset($property_data[$pattern]))
            {
                ...
                if(trim($value)) $rendered = str_replace('[' . $pattern . ']', $value, $rendered);
                else
                {
                    $rendered = str_replace('[' . $pattern . '][glue]', '', $rendered);
                    $rendered = str_replace('[' . $pattern . '] [glue]', '', $rendered);
                }
            }
        }
        ...
        $rendered = preg_replace('/('.$glue.'\s+)+/i', ',', $rendered);
    ...
    public static function get_pattern($pattern, $default, $kind = NULL, $property_type = NULL)
    {
        ...
        $seo_patterm = '';
        if(!is_null($kind) and !is_null($property_type))
        {
    ...
    public static function include_google_maps()
    {
        ...
        $javascript = (object) array('param1'=>'google-maps', 'param2'=>'http'.(stristr(wpl_global::get_full_url(), 'https://') != '' ? 's' : '').'://maps.google.com/maps/api/js?libraries=places,drawing'.(trim($API_key) != '' ? '&key='.$API_key : ''), 'external'=>true);
