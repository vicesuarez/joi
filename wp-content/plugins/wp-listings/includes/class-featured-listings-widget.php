		...
		echo $before_widget;
			...
			if ( $wp_listings_widget_query->have_posts() ) : while ( $wp_listings_widget_query->have_posts() ) : $wp_listings_widget_query->the_post();
				...
				if ( '' != get_post_meta( $post->ID, '_listing_bedrooms', true ) || '' != get_post_meta( $post->ID, '_listing_bathrooms', true ) || '' != get_post_meta( $post->ID, '_listing_sqft', true )) {
					$loop .= sprintf( '<ul class="listing-beds-baths-sqft"><li class="beds">%s<span>Habitaciones</span></li> <li class="baths">%s<span>Baños</span></li> <li class="sqft">%s<span>M<sup>2</sup></span></li></ul>', get_post_meta( $post->ID, '_listing_bedrooms', true ), get_post_meta( $post->ID, '_listing_bathrooms', true ), get_post_meta( $post->ID, '_listing_sqft', true ) );
				}
